...
c1 = 1
c2 = c1 - 1.73*h
N_v = (4/3)*np.pi*(a1*b1*c1 - a2*b2*c2)*np_avg
N_v=int(N_v)
mass = 1
...
elli = []
for i in range(N):
    if (((x[i,0]-center_p_x)/a1)**2 + ((y[i,0]-center_p_y)/b1)**2 + ((z[i,0]-center_p_z)/c1)**2) < 1:
        elli.append(i)
N = N-len(elli)

x = np.delete(x, elli, axis=0)
y = np.delete(y, elli, axis=0)
z = np.delete(z, elli, axis=0)
