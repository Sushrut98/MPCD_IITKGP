...
def vel_cm(locations, u, v):
	...
	count = np.zeros((nc_total,1),dtype=int)
	u_c = np.zeros((nc_total, 1),dtype=int)
	v_c = np.zeros((nc_total, 1),dtype=int)
	
	for j in range(0,N):
		list_particle[j,0] = int(head[locations[j,0],0])
		head[locations[j,0],0] = int(j)
