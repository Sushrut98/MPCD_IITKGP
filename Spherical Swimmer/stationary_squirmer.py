...
for i in range(0,Niter):
    ...
    #print('g')
    ## Anderson Thermostat
    u_r_at_f = np.random.normal(mu,sigma,(N,1))
    v_r_at_f = np.random.normal(mu,sigma,(N,1))
    w_r_at_f = np.random.normal(mu,sigma,(N,1))

    u_cm_r_f = cm_quantity(head, list_particle, count_f, u_r_at_f, nc_total)
    v_cm_r_f = cm_quantity(head, list_particle, count_f, v_r_at_f, nc_total)
    w_cm_r_f = cm_quantity(head, list_particle, count_f, w_r_at_f, nc_total)

    u_r_at_v = np.random.normal(mu,sigma,(N_v,1))
    v_r_at_v = np.random.normal(mu,sigma,(N_v,1))
    w_r_at_v = np.random.normal(mu,sigma,(N_v,1))

    u_cm_r_v = cm_quantity(head, list_particle, count_f, u_r_at_v, nc_total)
    v_cm_r_v = cm_quantity(head, list_particle, count_f, v_r_at_v, nc_total)
    w_cm_r_v = cm_quantity(head, list_particle, count_f, w_r_at_v, nc_total)

    u_si = cm_vel(u_cm_f + u_cm_v, count, nc_total)
    v_si = cm_vel(v_cm_f + v_cm_v, count, nc_total)
    w_si = cm_vel(w_cm_f + w_cm_v, count, nc_total)

    u = anderson_thermostat(u_cm, u_r_at_f, u_si, locations, N)
    v = anderson_thermostat(v_cm, v_r_at_f, v_si, locations, N)
    w = anderson_thermostat(w_cm, w_r_at_f, w_si, locations, N)
