...
# Streaming
def stream(a, a_u, dt, acc):
    a = a + a_u*dt+0.5*(acc**2)
    return a
...
for i in range(0,Niter):
    ...
    x[top_escape,0] = x[top_escape,0] - (np.multiply((2*(u[top_escape,0])), time_above_top)) + 0.5*acc_x*np.square(time_above_top)
    ...
    u[bottom_escape,0] = -u[bottom_escape,0] + 2*acc_x*time_above_top
