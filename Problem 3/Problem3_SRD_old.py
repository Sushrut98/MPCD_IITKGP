...
for i in range(0,Niter):
	...
	#     u_c[j,0] = np.sum(plt.scatter(x,y, s = 10, alpha = 0.8)plt.scatter(x,y, s = 10, alpha = 0.8)u[neighbour,0])/len(neighbour) #gives runtime warning because of NaN
	...
	for j in range(0,ncy):
		...
		if len(eta) == 0:
			...
			vel_data[j,i] = np.sum(u[same_eta,0])/len(eta)
